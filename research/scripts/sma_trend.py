"""Faber's moving-average timing over a small ETF universe, with GLD as the haven.

    uv run python scripts/sma_trend.py                     # fetch from Yahoo
    uv run python scripts/sma_trend.py --csv-dir DIR       # Kaggle-format CSVs, no network
    uv run python scripts/sma_trend.py --universe SPY,QQQ,BIL   # another universe, another haven

Each risk asset is held while it trades above its own trailing average and
swapped for the haven otherwise (docs/CONTRACTS.md, ``sma_trend``). The
lookback and the band around the average are chosen on the training window
from a fixed grid; the test window is touched once. The spec is promoted only
when the out-of-sample Sharpe clears 1.0 on Yahoo data.
"""
...
def main() -> int:
    ...
    aligned = trend.align(data.bars, universe)
    ...
    train_prices = trend.align(train_bars, universe)
    rows = []
