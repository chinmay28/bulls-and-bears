...
/** Meter is a labelled usage bar: the main way a guardrail is read at a glance. */
...
export function useLoader<T>(
  ...
  error: string | null
  loading: boolean
  /** True when Bulls and Bears could not be reached at all, as opposed to refusing. */
  offline: boolean
  reload: () => void
} {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Status 0 means the request never landed — Bulls and Bears is restarting,
 ...
 * Bulls and Bears being briefly unreachable is normal while it updates itself, so
...
export function Loading({
  ...
  if (offline) {
    return (
      <Banner tone="warn">
        Can't reach Bulls and Bears — reconnecting…
