...
// expected signals for the ETF/GLD ratio study (docs/PLAN.md §4.3).
const goldenDir = "../../../../golden/etf_gld_ratio"
...
func TestParity(t *testing.T) {
	...
	r, err := Parse(spec.Universe, strategy.Params(spec.Params), strategy.Sizing{GrossLeverage: spec.Sizing.GrossLeverage})
	if err != nil {
		t.Fatalf("spec.yaml: %v", err)
	}
	...
	for _, row := range rows {
		...
		if diff := math.Abs(got - row.weight); !(diff <= parityTolerance) {
			mismatches++
			if first == "" {
				first = fmt.Sprintf("%s %s: Go %.17g, Python %.17g, off by %.3g (Go state %v, z %v defined %v)",
					row.date, row.symbol, got, row.weight, diff, s.State, s.Z, s.ZDefined)
			}
		}
	}
	if mismatches > 0 {
